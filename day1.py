...
def solve_safe_dial(filename):
    ...
    Args:
        filename: Path to input file containing rotations
    
    Returns:
        Number of times dial points at 0 after rotations
    """
    # Start position
    position = 50
    count_zeros = 0
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            distance = int(line[1:])  # The number after L/R
            
            # Perform rotation
            if direction == 'L':
                # Left means subtract (towards lower numbers)
                position = (position - distance) % 100
            else:  # direction == 'R'
                # Right means add (towards higher numbers)
                position = (position + distance) % 100
            
            # Check if we landed on 0
            if position == 0:
                count_zeros += 1
                print(f"After {line}: position = {position} ★ (count = {count_zeros})")
            else:
                print(f"After {line}: position = {position}")
